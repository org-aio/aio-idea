...
use super::test_support::PluginStore;
...
async fn exercise_http(
    ...
    let permits = state.hold_publications().await?;
    ...
    assert!(state.has_test_package(&frontend.rev).await?);
    ...
    super::test_support::exercise_rollouts(
        state,
        base,
