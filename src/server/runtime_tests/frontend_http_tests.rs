...
use super::test_support::PluginPackage;
...
use super::test_support::{DiscoveredPlugin, PluginStore};
...
async fn frontend_http_mounts_verified_assets_and_revokes_live_access() -> Result<()> {
    ...
    let state = RuntimeState::isolated_admin_test(
        Arc::new(crate::host::ProductIdentity::new(identity.clone(), &database).await?),
        &database,
        &base,
    ...
    let result = exercise(&state, &base, &identity).await;
    server.abort();
    let _ = server.await;
    pool.close().await;
    result
...
async fn exercise(
    state: &RuntimeState,
    base: &str,
    identity: &aio_plugin_identity_server::IdentityService,
) -> Result<()> {
    ...
    assert_eq!(
        ...
        super::test_support::session_context(&session)
    );
    assert_eq!(
        mounted["data"]["context"],
        super::test_support::tenant_context(&session)?
    );
    assert!(
    ...
    let cached = super::test_support::prepare_frontend(state, &first.rev, "index.html").await?;
    let reused = super::test_support::prepare_frontend(state, &first.rev, "index.html").await?;
...
async fn activate(
    ...
    state.cache_test_package(package).await?;
    ...
    state
        ...
        .activate(
            ...
            DiscoveredPlugin {
                ...
                runtime: az_plugin_host::runtime::PluginRuntime::PageDefinition,
