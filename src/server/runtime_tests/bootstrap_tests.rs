...
async fn bootstrap_revalidates_permissions_and_logout_before_304() -> Result<()> {
    ...
    let state = super::RuntimeState::isolated_admin_test(
        std::sync::Arc::new(crate::host::ProductIdentity::new(identity.clone(), &database).await?),
        &database,
        &base,
        temporary.path(),
    )
...
async fn exercise(
    ...
    sqlx::query("INSERT INTO tenant_memberships (tenant_id, user_id, display_name) VALUES ($1, $2, 'Startup B')")
        .bind(&tenant).bind(&session.user_id).execute(state.database()).await?;
    assert!(identity.switch_tenant(&session, &tenant).await?);
    let switched = client
        .get(&url)
        .header(header::COOKIE, &cookie)
        .header(header::IF_NONE_MATCH, &first_etag)
