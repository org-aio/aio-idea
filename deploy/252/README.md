...
宿主平台 2026.9.21 提供受控 `POST /egress/responses`，固定转发到已授权模型基址的 `/responses`。先发布此宿主，再升级要求该最低版本的 Agent；密钥、模型目录、用户配对和设备权限均沿用现有配置。Agent 使用 Responses 的 input、函数输出项与完成事件，后台记忆整理也使用该协议。协议回归见 Agent 的 engine/README.md。
